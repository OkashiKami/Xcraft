...
import com.senpai.entity.EntityCentaur;
...
public class ChunkGeneratorFoz implements IChunkGenerator
{
	// Block that replaces End Stone
	protected static final IBlockState MAIN_BLOCK = BlockInit.DIRT.getDefaultState();
	...
	public ChunkGeneratorFoz(World world, boolean mapFeaturesEnabled, long seed, BlockPos spawn)
	    {
	        this.world = world;
	...
	public static float getIslandHeightValue(int x, int z, int p_185960_3_, int p_185960_4_) {
		...
		for (int i = -12; i <= 12; ++i) {
			for (int j = -12; j <= 12; ++j) {
				...
				if (k * k + l * l > 4096L && ChunkGeneratorFoz.islandNoise.getValue((double) k,
						(double) l) < -0.8999999761581421D) {
	...
	private double[] getHeights(double[] noiseField, int x, int y, int z, int xSize, int ySize, int zSize) {
		...
		if (noiseField == null)
			noiseField = new double[xSize * ySize * zSize];

		double d0 = 684.412D;
		double d1 = 684.412D;
		d0 = d0 * 2.0D;
		this.pnr = this.perlinNoise1.generateNoiseOctaves(this.pnr, x, y, z, xSize, ySize, zSize, d0 / 80.0D,
				4.277575000000001D, d0 / 80.0D);
	...
	public void populate(int x, int z) {
		...
		if (this.mapFeaturesEnabled) {
			...
			// e.g. this.endCityGen.generateStructure(this.world, this.rand, new ChunkPos(x,
			// z));
		}
			...
			if (this.getIslandHeightValue(x, z, 1, 1) > 40.0F) {
				...
				for (int k = 0; k < j; ++k) {
					...
					if (j1 > 0) {
						...
						if (this.world.isAirBlock(blockpos.add(l, k1 + 1, i1)) && this.world
								.getBlockState(blockpos.add(l, k1, i1)).getBlock() == MAIN_BLOCK.getBlock()) {
