...
import com.senpai.interfaces.IMetaBlockName;
 ...
 *
 */
public abstract class BlockMachine extends BlockBase implements IMetaBlockName, ITileEntityProvider {
	...
	public BlockMachine(String name) {
		...
		if(!this.getUnlocalizedName().equals(name)) this.setUnlocalizedName(name);
		if(this.getRegistryName().equals(name)) this.setRegistryName(Reference.MODID + "." + name);
		this.setHardness(3);
	...
	public int getMetaFromState(IBlockState state) {
		return ((ChipTypes)state.getValue(TYPE)).getMeta();
	}
	...
	public void updateMachineTier(World world, EntityPlayer player, EnumHand hand, BlockPos pos, ItemStack stack) {
		if(stack.getItem() == ItemInit.CHIP) {
