package com.senpai.network;

import com.senpai.interfaces.IWork;
...
public class PacketGetWorker implements IMessage {
	...
	public static class Handler implements IMessageHandler<PacketGetWorker, IMessage> {
		...
		void processMessage(PacketGetWorker message, MessageContext ctx) {
			...
			if (te == null)
				return;
			if (!te.hasCapability(CapabilitiesInit.CAPABILITY_WORKER, message.side))
				return;
			IWork worker = te.getCapability(CapabilitiesInit.CAPABILITY_WORKER, message.side);
