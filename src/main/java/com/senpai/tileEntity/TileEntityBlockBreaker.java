...
import com.senpai.capabilties.Worker;
import com.senpai.config.Config;
import com.senpai.init.BlockInit;
import com.senpai.init.CapabilitiesInit;
...
public class TileEntityBlockBreaker extends TileEntity implements ITickable, ICapabilityProvider {
	...
	public TileEntityBlockBreaker() {
		this.worker = new Worker(Config.machineCooldownBasic, () -> {
	...
	public void update() {
		if (this.world != null && !this.world.isRemote) { // Makes sure we have a world. RENAMED IN
			...
			if (!this.world.isBlockPowered(pos)) { // If the block is not powered
				if (!this.world.isAirBlock(pos) && this.world.getBlockState(pos).getBlock() == BlockInit.BREAKER) { // The block is not air and it is a block breaker
	...
	public void updateCooldownCap() {
		...
		if (this.world.getBlockState(pos).getValue(BlockBreaker.TYPE) == ChipTypes.BASIC)
			cap = Config.machineCooldownBasic;
		else
			cap = Config.machineCooldownAdvanced;
	...
	public <T> T getCapability(Capability<T> capability, EnumFacing facing) {
		...
		if(capability == CapabilitiesInit.CAPABILITY_WORKER)
	...
	public boolean hasCapability(Capability<?> capability, EnumFacing facing) {
		if (capability == CapabilityItemHandler.ITEM_HANDLER_CAPABILITY || capability == CapabilitiesInit.CAPABILITY_WORKER)
			return true;
