...
import com.senpai.util.StatList;
import com.senpai.util.handlers.EnumHandler;
import com.senpai.util.handlers.Mathf;
...
{
	public static final PropertyEnum<EnumHandler.AttachPosition> NORTH = PropertyEnum.<EnumHandler.AttachPosition>create("north", EnumHandler.AttachPosition.class);
    public static final PropertyEnum<EnumHandler.AttachPosition> EAST = PropertyEnum.<EnumHandler.AttachPosition>create("east", EnumHandler.AttachPosition.class);
    public static final PropertyEnum<EnumHandler.AttachPosition> SOUTH = PropertyEnum.<EnumHandler.AttachPosition>create("south", EnumHandler.AttachPosition.class);
    public static final PropertyEnum<EnumHandler.AttachPosition> WEST = PropertyEnum.<EnumHandler.AttachPosition>create("west", EnumHandler.AttachPosition.class);
    ...
    public BlockPipe(String name)
    {
        ...
        this.setDefaultState(this.blockState.getBaseState().withProperty(NORTH, EnumHandler.AttachPosition.NONE).withProperty(EAST, EnumHandler.AttachPosition.NONE).withProperty(SOUTH, EnumHandler.AttachPosition.NONE).withProperty(WEST, EnumHandler.AttachPosition.NONE).withProperty(POWER, Integer.valueOf(0)));
    }
    ...
    {
        int i = 0;
        boolean flag = state.getValue(NORTH) != EnumHandler.AttachPosition.NONE;
        boolean flag1 = state.getValue(EAST) != EnumHandler.AttachPosition.NONE;
        boolean flag2 = state.getValue(SOUTH) != EnumHandler.AttachPosition.NONE;
        boolean flag3 = state.getValue(WEST) != EnumHandler.AttachPosition.NONE;
    ...
    private EnumHandler.AttachPosition getAttachPosition(IBlockAccess worldIn, BlockPos pos, EnumFacing direction)
    {
        ...
        if (!canConnectTo(worldIn.getBlockState(blockpos), direction, worldIn, blockpos) && (iblockstate.isNormalCube() || !canConnectUpwardsTo(worldIn, blockpos.down())))
        {
            ...
            if (!iblockstate1.isNormalCube())
            {
                ...
                if (flag && canConnectUpwardsTo(worldIn, blockpos.up()))
                {
                    ...
                    {
                        return EnumHandler.AttachPosition.UP;
                    }

                    return EnumHandler.AttachPosition.SIDE;
                }
            }

            return EnumHandler.AttachPosition.NONE;
        }
        else
        {
            return EnumHandler.AttachPosition.SIDE;
    ...
    {
        return Item.getItemFromBlock(BlockInit.PIPE);
    }
